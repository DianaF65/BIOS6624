...
To determine whether subjects were adhering accurately to the +30 minute and +600 minute sampling times required by study protocol, intervals of time were defined to determine the number and percentage of patients that had good or adequate adherence.
Good adherence was defined by a collection of the saliva sample within a 7.5 minute window interval whereas adequate adherence if samples were within a 15 minute window. 
...
For the third hypothesis, we used a longitudinal linear mixed effects models to assess whether DHEA and Cortisol levels followed the expected diurnal pattern.
Due to a right-skew in the distribution of DHEA, a log transformed DHEA variable was used as the outcome to assess DHEA changes. 
No transformations was made to Cortisol. 
Covariates for oboth models include manually calculated pre and post 30 minute variables which would give us the slopes for the two time intervals of interest. 
...
We additionally assessed bias by.... COME BACK TO THIS
...
```{r, echo = F, warning = F, message = F, out.width="80%", fig.align='center', eval =F}
...
model_cortisol <- lme(`Cortisol (nmol/L)` ~ time_pre30 + time_post30,
                 random = ~ 1 | SubjectID,
                 method = "REML",
                 na.action = na.omit,
                 data = wake_data_q3)
